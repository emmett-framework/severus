...
class Translator:
    ...
    _re_langpath = re.compile(r"^[a-z]{2}([-_][a-zA-Z]{2})?(\.json|\.yml|\.yaml)?$")
    ...
    def __init__(
        ...
        if not issubclass(str_class, Tstr):
            raise RuntimeError(f"{str_class.__name__} should be a subclass of Tstr")
    ...
    def _build_languages(self):
        if self._path.is_dir():
            for path in self._path.iterdir():
                ...
                self._languages[lang_key] = Language(path, self._encoding, self._filename_prefix, self._changes_track)
        self._langmap[self._default_language] = self._langmap.get(self._default_language) or self._default_language
        self._languages[self._default_language] = self._languages.get(self._default_language) or Language(
            self._path / self._default_language, self._encoding, self._filename_prefix, self._changes_track
        )
    ...
    def _get_best_language(self, lang: Optional[str] = None) -> str:
        return self._langmap.get(lang or get_language(), self._default_language)

    def translate(self, text: str, lang: Optional[str] = None, *args, **kwargs) -> str:
        lang = self._get_best_language(lang)
        text_match, dict_match = self._languages[lang].get(text)
        n = kwargs.get("n")
        if n and dict_match:
            text_match = dict_match.get(max(i for i in dict_match.nkeys if i <= n), text_match)
        elif dict_match:
