...
class Language:
    ...
    def __init__(
        self, data_path: Path, encoding: str = "utf8", filename_prefix: bool = True, watch_changes: bool = False
    ):
    ...
    def _load_sources(self, path: Path, filename_prefix: bool = True):
        ...
        for source in sources:
            self._sources.append(
                {"path": source, "mtime": source.stat().st_mtime, "prefix": filename_prefix_applicable}
            )
    ...
    def _load_data(self, data: Dict[str, Union[Dict, str]], prefix: Optional[str] = None):
